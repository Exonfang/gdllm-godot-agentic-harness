...
func _test_long_file_map_and_ranged_read() -> void:
	...
	# Bounds stringified as floats must still count as a range — falling to the 0 fallback would read as "no range" and serve the map.
	var float_ranged: Dictionary = await GDLLMTools.execute("read_file", {"path": long_path, "start_line": "2.0", "end_line": "12.0"})
	_check(not float_ranged.has("subagent"), "string-float bounds still count as a range — no map detour")
	_check(String(float_ranged.get("content", "")).contains("(lines 2-12 of"), "string-float bounds read as their integer selves")
	_check(GDLLMTools._arg_int({"n": "6.0"}, ["n"], 0) == 6 and GDLLMTools._arg_int({"n": 6.0}, ["n"], 0) == 6, "_arg_int takes a float in either spelling")
	_check(GDLLMTools._arg_int({"n": "x"}, ["n"], 7) == 7, "a non-numeric string still falls back")
	_check(GDLLMTools.is_integer_like("6.0") and GDLLMTools.integer_like("6.0") == 6 and not GDLLMTools.is_integer_like("x"), "the shared integer-like judgment takes every numeric spelling")
	_check(not GDLLMTools.is_integer_like("0.9") and GDLLMTools._arg_int({"n": "0.9"}, ["n"], 7) == 7, "a fractional string is not integer intent — it falls back instead of silently truncating")
	_check(GDLLMTools.integer_like("9007199254740993") == 9007199254740993, "an integer-spelled string parses exactly, past float precision")
	_check(not GDLLMTools.is_integer_like("1e19") and GDLLMTools._arg_int({"n": "1e19"}, ["n"], 7) == 7, "a whole float past int range falls back instead of overflowing")
	_check(not GDLLMTools.is_integer_like(1e19) and GDLLMTools._arg_int({"n": 1e19}, ["n"], 7) == 7, "the RAW float spelling of the same number falls back too, not to int64 garbage")
	_check(not GDLLMTools.is_integer_like(6.5) and GDLLMTools.is_integer_like(6.0), "a raw fractional float keeps its refusal like its string spelling; a whole one passes")
	_check(GDLLMTools.is_index_like(2.0) and GDLLMTools.is_index_like("2") and not GDLLMTools.is_index_like("2.0"), "index-like narrows only the STRING float spelling, where a name could carry it")
	_check(GDLLMTools.number_like("1.5") == 1.5 and GDLLMTools.number_like("x", -1.0) == -1.0, "the float counterpart parses numeric strings and falls back")
	_check(GDLLMTools._arg_bool({"b": "1.0"}, ["b"]) and not GDLLMTools._arg_bool({"b": "0.0"}, ["b"]), "_arg_bool reads a stringified number's truthiness")
